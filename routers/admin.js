...
const {Prxwy} = require("../models/prxwy");
...
cloudinary.config({
    cloud_name: "docbvgook",
    api_key: "384724491755465",
    api_secret: "g3grxtSY9MLFWfs19yJyEywtM-A",
});
...
router.get(`/para`, ensureAuthenticated,  async (req, res) => {
	const para = await Para.find();
	
	res.render("admin/para", {
		para: para,
		user: req.user,
	});
});

router.get(`/prxwy`, ensureAuthenticated,  async (req, res) => {
	const prxwy = await Prxwy.find();
	
	res.render("admin/prxwy", {
		prxwy: prxwy,
		user: req.user,
	});
});
...
router.post(`/addcategory`, upload.single("image"), ensureAuthenticated, async (req, res) => {
    ...
    if (!category) res.status(500).send("This category was not sent to database...");

    res.redirect("/admin/addcategory");
});
router.get(`/addpara`, ensureAuthenticated, async (req, res) => {

	res.render("admin/addpara", {
		user: req.user,
	});
});
router.get(`/addprxwy`, ensureAuthenticated, async (req, res) => {

	res.render("admin/addprxwy", {
		user: req.user,
	});
});
...
router.post(`/addpara`, ensureAuthenticated, async (req, res) => {
	...
	para = await para.save();

	if (!para)
		res.status(500).send("This para was not sent to database...");

	res.redirect("/admin/addpara");
});
router.post(`/addprxwy`, ensureAuthenticated, async (req, res) => {
	...
	prxwy = await prxwy.save();

	if (!prxwy)
		res.status(500).send("This prxwy was not sent to database...");

	res.redirect("/admin/addprxwy");
});

router.get("/updatepara", ensureAuthenticated, async (req, res) => {
	const para = await Para.findOne({ _id: req.query.id }).populate(
		// "subcategory","product"
	);
	// const subcategory = await subCategory.find().select("name");
	// const product = await Product.find().select("name");

	res.render("admin/updatepara", {
		// subcategory: subcategory,
		para: para,
		user: req.user,
		// product: product,
...
router.get("/updateprxwy", ensureAuthenticated, async (req, res) => {
	const prxwy = await Prxwy.findOne({ _id: req.query.id }).populate(
		// "subcategory","product"
	);
	// const subcategory = await subCategory.find().select("name");
	// const product = await Product.find().select("name");

	res.render("admin/updateprxwy", {
		// subcategory: subcategory,
		prxwy: prxwy,
		user: req.user,
		// product: product,
...
router.post(
	...
	async (req, res) => {
		...
		if (file) {
			...
			const para = await Para.findById(req.params.id);
			imagePath = para.image;
		}

		const updatePara = await Para.findByIdAndUpdate(
			req.params.id,
			{
				name: req.body.name,
...
router.post(
	...
	async (req, res) => {
		...
		if (file) {
			...
			const prxwy = await Prxwy.findById(req.params.id);
			imagePath = prxwy.image;
		}

		const updatePrxwy = await Prxwy.findByIdAndUpdate(
			req.params.id,
			{
				name: req.body.name,
				// price: req.body.price,
...
router.post(`/addsubcategory`, ensureAuthenticated, async (req, res) => {
    ...
    if (!subcategory) res.status(500).send("This subcategory was not sent to database...");

    res.redirect("/admin/addsubcategory");
});

router.get("/updatesubcategory", ensureAuthenticated, async (req, res) => {
    const subcategory = await subCategory.findOne({ _id: req.query.id }).populate("category");
...
router.post(`/addproduct`, ensureAuthenticated, upload.single("image"), async (req, res) => {
    ...
    if (!product) res.status(500).send("This product was not sent to database...");

    res.redirect("/admin/addproduct");
});

router.get("/updateproduct", ensureAuthenticated, async (req, res) => {
    const product = await Product.findOne({ _id: req.query.id }).populate("subcategory", "category");
    const subcategory = await subCategory.find().select("name");
    const category = await Category.find().select("name");
...
router.post("/updatesubcategory/:id", ensureAuthenticated, upload.single("image"), async (req, res) => {
    if (!mongoose.isValidObjectId(req.params.id)) {
...
router.post("/updateproduct/:id", ensureAuthenticated, upload.single("image"), async (req, res) => {
    if (!mongoose.isValidObjectId(req.params.id)) {
...
router.post("/updatecategory/:id", ensureAuthenticated, upload.single("image"), async (req, res) => {
    if (!mongoose.isValidObjectId(req.params.id)) {
        return res.status(400).send("Invalid Category id");
    }
